...
export const TopBar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getItems<Noticia>('noticias')
      ...
      .catch((err) => console.error('Error fetching marquee news:', err));
    return () => {
      active = false;
    };
  }, []);

  // Repeat the news list to create an infinite loop effect for the marquee
  const marqueeItems = [...noticias, ...noticias, ...noticias];

  return (
    <div className="bg-brand-blue text-white text-xs font-sans font-medium border-b border-white/10 py-2.5 px-4 select-none">
      <div className="max-w-[1360px] mx-auto flex justify-between items-center gap-6">

        {/* Left Side: Marquee with Latest News */}
        <div className="flex-1 flex items-center min-w-0 gap-3">
          {/* Label Badge */}
          <div className="bg-brand-red text-white text-3xs font-black uppercase px-2 py-0.5 rounded-xs tracking-wider shrink-0 select-none shadow-sm flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-brand-yellow animate-pulse-live" />
            <span>ÚLTIMAS NOTÍCIAS</span>
          </div>

          {/* Marquee Content */}
          <div className="flex-1 overflow-hidden relative flex items-center h-5 select-none">
            <div
              className="animate-marquee hover:[animation-play-state:paused] flex gap-10 items-center cursor-pointer"
            ...
            >
              {marqueeItems.map((item, idx) => (
                <Link
                  ...
                  className="hover:text-brand-yellow transition-colors font-bold text-white/90 flex items-center gap-2 text-2xs shrink-0"
                >
                  <span className="w-1 h-1 bg-brand-yellow rounded-full shrink-0" />
